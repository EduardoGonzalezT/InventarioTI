...
import { getPantallas } from "../../services/equiposServices";
...
onMounted(async () => {
  try {
    equipos.value = await getPantallas();
    console.log(equipos.value);
  } catch (error) {
    console.log("Error al obtener pantallas:", error.response?.data || error.message);
...
<template>
  <Toolbar>
    ...
    <template #center>
      <small><strong>Listado de Equipos de Pantallas</strong></small>
    </template>
