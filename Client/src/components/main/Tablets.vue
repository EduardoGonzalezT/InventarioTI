...
import { getTablets } from "../../services/equiposServices";
...
onMounted(async () => {
  try {
    equipos.value = await getTablets();
    console.log(equipos.value);
  } catch (error) {
    console.log("Error al obtener tablets:", error.response?.data || error.message);
    alert("Error al obtener tablets");
  }
});
</script>
...
<template>
  <Toolbar>
    ...
    <template #center>
      <small><strong>Listado de Equipos de Tablets</strong></small>
    </template>
