...
import { getServidores } from "../../services/equiposServices";
...
onMounted(async () => {
  try {
    equipos.value = await getServidores();
    console.log(equipos.value);
  } catch (error) {
    console.log("Error al obtener servidores:", error.response?.data || error.message);
...
<template>
  <Toolbar>
    ...
    <template #center>
      <small><strong>Listado de Equipos de Servidores</strong></small>
    </template>
