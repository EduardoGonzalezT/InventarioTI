...
import { getGrabadoras } from "../../services/equiposServices";
...
onMounted(async () => {
  try {
    equipos.value = await getGrabadoras();
    console.log(equipos.value);
  } catch (error) {
    console.log("Error al obtener grabadoras:", error.response?.data || error.message);
    alert("Error al obtener grabadoras");
  }
});
</script>
...
<template>
  <Toolbar>
    ...
    <template #center>
      <small><strong>Listado de Equipos de Grabadoras</strong></small>
    </template>
