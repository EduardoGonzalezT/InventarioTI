...
<template>
  <div class="cardEquipo" :key="computadora.Id">
    <div class="cardTop">
      <div class="cardTopLeft">
        <div class="IconoEquipo">
          <i class="pi pi-desktop" style="font-size: 2rem"></i>
          <div class="NomCodEquipo">
            <small>
              <strong>{{ computadora.Codigo }}</strong>
            </small>
            <small>{{ computadora.Nombre }}</small>
            <small>{{ computadora.Descripcion }}</small>
          </div>
        </div>
        <div class="infoEquipo">
          <div class="infoTexts">
            <small><strong>Area:</strong></small>
            <small>{{ computadora["area.Area"] }}</small>
          </div>
          <div class="infoTexts">
            <small><strong>Estatus:</strong></small>
            <Badge
              :value="Status(computadora.Status)"
              :severity="computadora.Status == 1 ? 'success' : 'danger'"
              size="small"
            ></Badge>
          </div>
        </div>
      ...
      <div class="cardTopRight">
        <router-link :to="`/equipos/ComputadoraDetalles?id=${computadora.Id}`" class="btnVerMas">
          <svg width="40" height="40" viewBox="0 0 24 24">
            <path
              fill="currentColor"
              d="M13.292 12L9.046 7.754q-.14-.14-.15-.344q-.01-.204.15-.364t.354-.16q.194 0 .354.16l4.388 4.389q.131.13.184.267q.053.136.053.298t-.053.298q-.053.137-.184.267l-4.388 4.389q-.14.14-.344.15q-.204.01-.364-.15t-.16-.354q0-.194.16-.354L13.292 12Z"
            />
          </svg>
        </router-link>
      </div>
    ...
    <div class="cardBottom">
      ...
      <small>{{ computadora["sucursal.Descripcion"] }}</small>
    </div>
  </div>
</template>
