...
import { getImpresoras } from "../../services/equiposServices";
...
onMounted(async () => {
  try {
    equipos.value = await getImpresoras();
    console.log(equipos.value);
  } catch (error) {
    console.log("Error al obtener impresoras:", error.response?.data || error.message);
    alert("Error al obtener impresoras");
  }
});
</script>
...
<template>
  <Toolbar>
    ...
    <template #center>
      <small><strong>Listado de Equipos de Impresion</strong></small>
    </template>
