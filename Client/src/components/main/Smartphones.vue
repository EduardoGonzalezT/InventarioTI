...
import { getSmartPhones } from "../../services/equiposServices";
...
onMounted(async () => {
  try {
    equipos.value = await getSmartPhones();
    console.log(equipos.value);
  } catch (error) {
    console.log("Error al obtener smartphones:", error.response?.data || error.message);
    alert("Error al obtener smartphones");
  }
});
</script>
...
<template>
  <Toolbar>
    ...
    <template #center>
      <small><strong>Listado de Equipos de Smartphones</strong></small>
    </template>
